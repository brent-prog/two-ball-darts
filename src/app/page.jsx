  ...
  return (
    <main className="app-shell">
      ...
      <section className="card" id="scorecard">
        <div className="section-heading"><div><p className="eyebrow">Live round</p><h2>Golf-style scorecard</h2></div><div className="actions-inline"><button className="button secondary" onClick={() => setPlayers([...players, { id: crypto.randomUUID(), name: `Player ${players.length + 1}`, scores: {} }])}>Add player</button><button className="button ghost" onClick={() => setPlayers(players.map(p => ({ ...p, scores: {} })))}>Reset</button><button className="button primary" onClick={saveRound}>Save round</button></div></div>
        <div className="hole-picker">{holes.map(hole => <button key={hole} className={hole === activeHole ? 'active' : ''} onClick={() => setActiveHole(hole)}>{hole}</button>)}</div>
        <div className="active-hole-panel"><h3>Hole {activeHole}</h3><p>Pick each player's result for the current hole.</p><div className="player-score-grid">{players.map(player => <div className="player-hole-card" key={player.id}><input value={player.name} onChange={e => updateName(player.id, e.target.value)} /> <div className="score-buttons">{scoreResults.map(result => <button key={result.key} className={player.scores[activeHole] === result.key ? 'selected' : ''} onClick={() => updateScore(player.id, result.key)}><span>{result.label}</span><strong>{fmt(result.score)}</strong></button>)}<button className="clear-score" onClick={() => updateScore(player.id, '')}>Clear</button></div></div>)}</div></div>
      ...
      <section className="two-column"><div className="card" id="rules"><p className="eyebrow">Official rule assistant</p><h2>Ask a rule question</h2><div className="rule-input"><input value={question} onChange={e => setQuestion(e.target.value)} placeholder="Example: single target plus off board?"/><button className="button primary" onClick={askRule}>Ask</button></div><div className="rule-answer"><h3>{answer.title}</h3><p>{answer.answer}</p><span>Matched rule: {answer.matchedRule}</span></div></div><div className="card"><div className="section-heading compact"><div><p className="eyebrow">Supabase history</p><h2>Saved rounds</h2></div><button className="button secondary" onClick={loadHistory}>Load</button></div><div className="history-list">{history.map(game => <div className="history-row" key={game.id}><strong>{game.title}</strong><span>{new Date(game.played_at).toLocaleString()} · {game.course_name}</span></div>)}</div></div></section>
    </main>
  );
}
