    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 300, background: 'rgba(0,0,0,.78)', padding: '20px', display: 'grid', placeItems: 'center' }}>
      <div className="card" style={{ width: 'min(760px, 96vw)', maxHeight: '90vh', overflow: 'auto', margin: 0, borderColor: '#d0a948' }}>
        <div className="section-heading compact" style={{ marginBottom: '14px', alignItems: 'flex-start' }}>
          <div><p className="eyebrow">Quick start</p><h2 style={{ fontSize: 'clamp(2rem, 6vw, 3.8rem)' }}>How to Play</h2></div>
          ...
          <div style={{ display: 'grid', gap: '8px' }}>
            {cheatRows.map(([label, description, score]) => (
              <div key={label} style={{ display: 'grid', gridTemplateColumns: 'minmax(110px, 150px) 1fr 58px', gap: '10px', alignItems: 'center', border: '1px solid rgba(208,169,72,.45)', borderRadius: '12px', padding: '10px', background: 'rgba(0,0,0,.18)' }}>
                <strong style={{ color: '#fff4d6' }}>{label}</strong><span style={{ lineHeight: 1.35 }}>{description}</span><strong style={{ color: '#d0a948', textAlign: 'right' }}>{score}</strong>
              </div>
            ))}
          </div>
        </div>
...
export default function Home() {
  ...
  function resetRound() { setSavedGameId(null); setIsRoundDirty(true); setStatus(''); setShowLiveScorecard(false); setPlayers(current => current.map(player => ({ ...player, scores: {} }))); }
  function removePlayer(playerId) { markRoundDirty(); setPlayers(current => current.length <= 1 ? current : current.filter(player => player.id !== playerId)); }
  ...
  function updateRuleDart(which, value) {
    ...
    setAnswer(response); setShowAllRules(false); setShowAddToScore(false); setShowRuleHolePicker(false); setRulePlayerId(''); setRuleHole(activeHole);
    if (nextDartOne && nextDartTwo) supabase.from('rule_questions').insert({ owner_key: getOwnerKey(), question: `Dart 1: ${nextDartOne}; Dart 2: ${nextDartTwo}`, matched_rule: response.matchedRule, answer: response.answer }).then(() => undefined);
  }
  function openAddToScore() { setRuleHole(activeHole); setShowRuleHolePicker(false); setShowAddToScore(current => !current); }
  ...
  async function saveRound() {
    ...
    } catch (error) { if (createdNewGame) await cleanupFailedGame(gameId); setStatus(`Save failed: ${error.message}`); } finally { setIsSaving(false); }
  }
  ...
  return (
    <main className="app-shell">
      {!showLiveMode && <section className="hero-card"><LogoMark /><div className="hero-copy"><p className="eyebrow">18 holes · 2 darts per hole · all holes par 3</p><h1>No gimmes. Just throw.</h1><p>Golf-course scoring for the dartboard. Play holes 1 through 18 by throwing two darts at each number. Lowest score wins.</p><div className="hero-actions"><button className="button primary" onClick={() => setShowLiveMode(true)}>Enter Live Mode</button><a href="#scorecard" className="button secondary">Start scoring</a><button className="button secondary" onClick={() => setShowHowToPlay(true)}>How to Play</button><a href="#rules" className="button secondary">Check rules</a></div></div></section>}
      ...
      <section className="quick-stats" aria-label="Round status and navigation" style={{ position: 'sticky', top: '10px', zIndex: 50, display: 'grid', gridTemplateColumns: '72px repeat(4, minmax(110px, 1fr)) minmax(220px, auto)', alignItems: 'center', backdropFilter: 'blur(12px)' }}>
        ...
        <div className="quick-nav" style={{ display: 'flex', gap: '10px', alignItems: 'center', justifyContent: 'flex-end', flexWrap: 'wrap', borderRight: 0, padding: '10px 12px' }}>{showLiveMode ? <><button className="button secondary" style={{ padding: '9px 14px' }} onClick={() => setShowHowToPlay(true)}>How to Play</button><button className="button primary" style={{ padding: '9px 14px' }} onClick={() => setShowLiveMode(false)}>Exit Live Mode</button></> : <><a href="#scorecard" className="button secondary" style={{ padding: '9px 14px' }}>Scorecard</a><a href="#rules" className="button primary" style={{ padding: '9px 14px' }}>Check Rules</a></>}</div>
      </section>

      <section className="card" id="scorecard" style={{ paddingTop: showLiveMode ? '22px' : '18px' }}>
        <div className="section-heading" style={{ marginBottom: '12px', alignItems: 'center' }}>
          <div><p className="eyebrow" style={{ margin: 0 }}>{showLiveMode ? 'Live mode' : 'Live round'}</p><p style={{ margin: '6px 0 0', color: '#fff4d6', fontWeight: 900 }}>Choose the current hole, score each player's two darts, then move to the next hole.</p></div>
          <div className="actions-inline"><button className="button secondary" onClick={() => setShowLiveMode(current => !current)}>{showLiveMode ? 'Exit Live Mode' : 'Enter Live Mode'}</button><button className="button secondary" onClick={addPlayer}>Add player</button><button className="button ghost" onClick={resetRound}>Reset</button><button className="button primary" disabled={isSaving || (Boolean(savedGameId) && !isRoundDirty)} onClick={saveRound}>{saveButtonLabel()}</button></div>
        </div>
        <div style={{ display: 'flex', gap: '10px', alignItems: 'stretch', marginBottom: '12px', flexWrap: 'wrap' }}><button className="button secondary" disabled={activeHole === 1} onClick={goToPreviousHole} style={{ opacity: activeHole === 1 ? .45 : 1 }}>Previous Hole</button><div style={{ flex: '1 1 220px', minWidth: '180px', border: '1px solid rgba(208,169,72,.55)', borderRadius: '14px', padding: '10px 14px', background: 'rgba(0,0,0,.18)', textAlign: 'center' }}><span style={{ display: 'block', color: '#d0a948', fontSize: '.78rem', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '.08em' }}>Current hole</span><strong style={{ display: 'block', marginTop: '3px', fontSize: '1.35rem' }}>Hole {activeHole} of 18</strong></div><button className="button primary" disabled={activeHole === 18} onClick={goToNextHole} style={{ opacity: activeHole === 18 ? .45 : 1 }}>Next Hole</button></div>
        ...
        <div className="active-hole-panel" style={{ padding: '14px', marginBottom: '16px' }}><h3 style={{ fontSize: '1.55rem', marginBottom: '10px' }}>Hole {activeHole}</h3><div className="player-score-grid">{players.map(player => <div className="player-hole-card" key={player.id}><div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: '8px', marginBottom: '8px' }}><input style={{ padding: '10px', marginBottom: 0 }} value={player.name} onFocus={e => e.target.select()} onChange={e => updateName(player.id, e.target.value)} /><button className="button ghost" style={{ padding: '8px 12px', borderRadius: '10px', opacity: players.length <= 1 ? .45 : 1 }} disabled={players.length <= 1} onClick={() => removePlayer(player.id)}>Remove</button></div><div className="score-buttons">{scoreResults.map(result => <button key={result.key} style={{ minHeight: '42px', padding: '8px 10px' }} className={player.scores[activeHole] === result.key ? 'selected' : ''} onClick={() => updateScore(player.id, result.key)}><span>{result.label}</span><strong>{fmt(result.score)}</strong></button>)}<button className="clear-score" style={{ minHeight: '40px', padding: '8px 10px' }} onClick={() => updateScore(player.id, '')}>Clear</button></div></div>)}</div>{isActiveHoleComplete && <div style={{ display: 'flex', gap: '12px', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', marginTop: '16px', border: '2px solid rgba(208,169,72,.72)', borderRadius: '16px', padding: '14px', background: 'linear-gradient(135deg, rgba(208,169,72,.18), rgba(6,57,39,.62))' }}><div><strong style={{ display: 'block', fontSize: '1.25rem', color: '#fff4d6' }}>{activeHole === 18 ? 'Round complete' : `Hole ${activeHole} complete`}</strong><span style={{ display: 'block', marginTop: '4px', color: '#d0a948', fontWeight: 900 }}>{activeHole === 18 ? 'Every player has a score for 18.' : 'Every player has a score for this hole.'}</span></div>{activeHole < 18 ? <button className="button primary" onClick={goToNextHole}>Next Hole</button> : <button className="button primary" disabled={isSaving || (Boolean(savedGameId) && !isRoundDirty)} onClick={saveRound}>{saveButtonLabel()}</button>}</div>}</div>
        <div style={{ display: 'flex', gap: '12px', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', marginBottom: showLiveScorecard ? '12px' : 0 }}><div style={{ color: '#d0a948', fontWeight: 900 }}>Scorecard is hidden during live scoring to keep entry fast.</div><button className="button secondary" onClick={() => setShowLiveScorecard(current => !current)}>{showLiveScorecard ? 'Hide Scorecard' : 'Show Scorecard'}</button></div>
        {showLiveScorecard && <LiveScorecard players={players} />}{status && <p className="status-line">{status}</p>}
      </section>

      {!showLiveMode && <section className="two-column"><div className="card" id="rules"><p className="eyebrow">Official rule assistant</p><h2>Check a two-dart result</h2><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(210px, 1fr))', gap: '12px', margin: '14px 0' }}><label style={{ display: 'grid', gap: '8px', fontWeight: 900, color: '#d0a948', textTransform: 'uppercase', letterSpacing: '.06em' }}>Dart 1<select value={dartOne} onChange={e => updateRuleDart('one', e.target.value)} style={{ width: '100%', borderRadius: '12px', border: '2px solid #d0a948', background: '#02140f', color: '#fff4d6', padding: '14px', fontWeight: 900 }}>{dartOptions.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label><label style={{ display: 'grid', gap: '8px', fontWeight: 900, color: '#d0a948', textTransform: 'uppercase', letterSpacing: '.06em' }}>Dart 2<select value={dartTwo} onChange={e => updateRuleDart('two', e.target.value)} style={{ width: '100%', borderRadius: '12px', border: '2px solid #d0a948', background: '#02140f', color: '#fff4d6', padding: '14px', fontWeight: 900 }}>{dartOptions.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label></div>{showAllRules ? <AllRulesPanel /> : <div className="rule-answer"><h3>{answer.title}</h3><div className="rule-result-copy"><p>{answerDescription}</p>{answerResult && <p className="rule-result-score">Score: <strong>({fmt(answerResult.score)})</strong> · Strokes: <strong>{answerResult.strokes}</strong></p>}</div><span>Matched rule: {answer.matchedRule}</span></div>}{canAddRuleResult && !showAllRules && <button className="button secondary" style={{ marginTop: '14px', marginRight: '10px', boxShadow: '0 0 0 3px rgba(208,169,72,.16)' }} onClick={openAddToScore}>Add to Score</button>}{showAddToScore && canAddRuleResult && !showAllRules && <div className="rule-answer" style={{ marginTop: '14px', borderLeftColor: '#d0a948' }}><h3>Add {answer.title} to score</h3><p>Selected scoring hole: <strong>Hole {ruleHole}</strong></p><button className="button secondary" style={{ marginBottom: '12px' }} onClick={() => setShowRuleHolePicker(current => !current)}>{showRuleHolePicker ? 'Hide Hole Picker' : 'Change Hole'}</button>{showRuleHolePicker && <div className="hole-picker" style={{ marginBottom: '14px', gridTemplateColumns: 'repeat(6, 1fr)' }}>{holes.map(hole => <button key={hole} className={hole === ruleHole ? 'active' : ''} onClick={() => setRuleHole(hole)}>{hole}</button>)}</div>}<label style={{ display: 'grid', gap: '8px', fontWeight: 900, color: '#d0a948', textTransform: 'uppercase', letterSpacing: '.06em' }}>Player<select value={rulePlayerId} onChange={e => setRulePlayerId(e.target.value)} style={{ width: '100%', borderRadius: '12px', border: '2px solid #d0a948', background: '#02140f', color: '#fff4d6', padding: '14px', fontWeight: 900 }}><option value="">Select player</option>{players.map(player => <option key={player.id} value={player.id}>{player.name}</option>)}</select></label><button className="button primary" style={{ marginTop: '12px' }} disabled={!rulePlayerId} onClick={addRuleResultToScore}>Confirm Hole {ruleHole}</button></div>}<button className="button primary" style={{ marginTop: '14px', boxShadow: '0 0 0 3px rgba(208,169,72,.28)' }} onClick={() => setShowAllRules(current => !current)}>{showAllRules ? 'Hide All Rules' : 'Display All Rules'}</button></div><div className="card"><div className="section-heading compact"><div><p className="eyebrow">Supabase history</p><h2>Saved rounds</h2></div><button className="button secondary" onClick={() => loadHistory()}>Load</button></div>{historyStatus && <p className="status-line">{historyStatus}</p>}<div className="history-list">{history.map(game => <div className="history-row" key={game.id}><strong>{game.title}</strong><span>{new Date(game.played_at).toLocaleString()} · {savedRoundLabel(game)}</span><span>{savedRoundSummary(game)}</span><button className="button primary" style={{ marginTop: '10px' }} onClick={() => viewSavedGame(game)}>View Scorecard</button></div>)}</div></div></section>}

      {!showLiveMode && <footer style={{ marginTop: '22px', border: '2px solid rgba(208,169,72,.72)', borderRadius: '22px', padding: '18px', background: 'linear-gradient(180deg, rgba(6,57,39,.88), rgba(2,20,15,.96))', boxShadow: '0 18px 44px rgba(0,0,0,.35)' }}><div style={{ display: 'grid', gridTemplateColumns: '72px 1fr auto', gap: '16px', alignItems: 'center' }}><img src="/two-ball-darts-logo.png" alt="TWO BALL DARTS" style={{ width: '64px', height: '64px', objectFit: 'contain', display: 'block' }} /><div><strong style={{ display: 'block', fontSize: '1.35rem', letterSpacing: '.03em' }}>TWO BALL DARTS</strong><span style={{ color: '#d0a948', fontWeight: 900 }}>No gimmes. Just throw.</span><p style={{ margin: '6px 0 0', color: '#f5e8c6' }}>18 holes. Two darts per hole. Bulls count for nothing - ever.</p></div><div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', justifyContent: 'flex-end' }}><a href="#scorecard" className="button secondary" style={{ padding: '9px 14px' }}>Scorecard</a><a href="#rules" className="button primary" style={{ padding: '9px 14px' }}>Rules</a></div></div></footer>}
