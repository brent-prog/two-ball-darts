    ...
    <div className="card" style={{ width: 'min(1120px,96vw)', maxHeight: '90vh', overflow: 'auto', margin: 0, borderColor: '#d0a948' }}>
      <div className="section-heading compact" style={{ marginBottom: '14px', alignItems: 'flex-start' }}>
        <div>
          ...
          <h2 style={{ fontSize: 'clamp(1.8rem,6vw,3rem)' }}>{new Date(game.played_at).toLocaleDateString()}</h2>
        </div>
      ...
      <div className="scorecard-table-wrap">
        <table className="scorecard-table">
          ...
          <tbody>{(game.game_players ?? []).map(row => <tr key={row.id}>
            <th>{row.players?.display_name || 'Player'}</th>
            <td className="total-score">{fmt(row.total_score)}</td>
            {holes.map(hole => <td key={hole}>{scoreLabel(scoreForHole(row, hole))}</td>)}
          </tr>)}</tbody>
        </table>
      </div>
    </div>
...
function RoundCard({ game, accountPlayerId, onView }) {
  ...
  return <article style={{ border: '1px solid rgba(208,169,72,.5)', borderRadius: '20px', padding: '16px', background: 'linear-gradient(180deg, rgba(6,57,39,.72), rgba(2,20,15,.9))', boxShadow: '0 12px 30px rgba(0,0,0,.2)' }}>
    ...
    <div style={{ marginTop: '14px', padding: '12px 14px', borderRadius: '15px', background: winners.length ? 'rgba(208,169,72,.16)' : 'rgba(255,255,255,.04)', border: '1px solid rgba(208,169,72,.32)' }}>
      {winners.length ? <>
        <span style={{ display: 'block', fontSize: '.72rem', fontWeight: 900, color: '#d0a948', textTransform: 'uppercase', letterSpacing: '.09em' }}>{winners.length > 1 ? 'Tie' : 'Winner'}</span>
        <strong style={{ display: 'block', marginTop: '3px', color: '#fff4d6', fontSize: '1.35rem' }}>{winnerNames.join(' & ')} <span style={{ color: '#d0a948' }}>{winnerScore}</span></strong>
      </> : <strong style={{ color: '#fff4d6' }}>Round in progress</strong>}
    </div>
    ...
    <div style={{ display: 'grid', gap: '8px', marginTop: '14px' }}>
      {players.map(row => {
        ...
        return <div key={row.id} style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) auto', gap: '12px', alignItems: 'center', padding: '9px 0', borderBottom: '1px solid rgba(255,244,214,.09)' }}>
          <span style={{ fontWeight: isWinner || isMe ? 900 : 700, color: isWinner ? '#fff4d6' : 'inherit' }}>{row.players?.display_name || 'Player'}{isMe ? ' · You' : ''}</span>
          <strong style={{ fontSize: '1.08rem', color: isWinner ? '#d0a948' : '#fff4d6' }}>{fmt(row.total_score)}</strong>
        </div>;
      })}
    </div>

    {stats && <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,minmax(0,1fr))', gap: '8px', marginTop: '14px' }}>
      <StatChip label="Par+" value={`${stats.parBetterPct}%`} />
      <StatChip label="Birdies+" value={stats.birdiesPlus} />
      <StatChip label="Bogeys+" value={stats.bogeysPlus} />
    </div>}
  </article>;
}
...
export default function RoundHistoryCards() {
  ...
  async function loadGames() {
    ...
    if (roundGames.length) {
      ...
      const { data: rows } = await supabase
        .from('game_players')
        .select('id,game_id,player_id,display_order,total_score,total_strokes,players(id,display_name,is_profile),hole_scores(hole_number,relative_score,strokes,result)')
        .in('game_id', ids)
        .order('display_order', { ascending: true });

      roundGames = roundGames.map(game => ({ ...game, game_players: (rows ?? []).filter(row => row.game_id === game.id) }))
        .filter(game => game.game_players.length > 0);
    }
  ...
  return createPortal(<>
    <div data-tbd-rich-round-history style={{ display: 'grid', gap: '12px', marginTop: '4px' }}>
      ...
      {visibleGames.map(game => <RoundCard key={game.id} game={game} accountPlayerId={accountPlayerId} onView={setSelectedGame} />)}
