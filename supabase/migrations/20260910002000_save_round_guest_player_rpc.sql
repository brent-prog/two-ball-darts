create or replace function public.save_two_ball_round_guest(
...
begin
  ...
  if v_guest.id is null then
    ...
    set display_name = v_name,
        owner_profile_id = coalesce(v_profile_id, owner_profile_id)
    where public.players.id = v_guest.id
    returning * into v_guest;
  end if;
...
revoke all on function public.save_two_ball_round_guest(text,text) from public;
grant execute on function public.save_two_ball_round_guest(text,text) to anon, authenticated;
